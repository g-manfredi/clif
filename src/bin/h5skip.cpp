...
int main(const int argc, const char *argv[])
{
  ...
  m_out.create(m_in.type(), {m_in[0], m_in[1], m_in[2], (m_in[3]+1)/2});
  int i = 0;
  for(auto pos :  Idx_It_Dims(m_in, 2, m_in.size()-1)) {
    ...
    if (i % 2 == 0) {
      ...
      out_bound = out_bound.bind(m_in.size()-1, pos[m_in.size()-1]/2);
      for(int dim=m_in.size()-2;dim>=2;dim--)
        out_bound = out_bound.bind(dim, pos[dim]);
    
      cvMat(in_bound).copyTo(cvMat(out_bound));
    }
  }
